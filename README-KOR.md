...
`openai-ralph-codex`는 Codex 안에서 더 긴 작업을 안정적으로 굴리기 위한
PRD 기반 Ralph 루프 플러그인 + CLI입니다.

핵심 아이디어는 단순합니다.

- **Codex**가 실제 작업을 수행하고
- **Ralph**가 작업을 PRD/태스크/검증 루프로 구조화하며
- **`.ralph/`**가 그 프로젝트 전용 상태를 보관합니다

즉, Codex를 대체하는 도구가 아니라 **큰 작업을 더 안전하게 진행하기 위한
구조를 Codex 위에 얹는 도구**입니다.
...
- 기능 단위 이상의 작업
- 한 번의 프롬프트로 끝내기 어려운 작업
- 검증이 꼭 필요한 작업
- 막힘 / 재시도 / 재개가 자연스럽게 있어야 하는 작업
- PRD나 작은 태스크들로 나눠서 진행하는 게 더 안전한 작업

반대로 아주 작은 수정, 짧은 질답, 일회성 응답은 그냥 Codex만 써도 충분합니다.
...
- `orc` CLI가 설치되고
- 기존 호환용 `ralph` alias도 유지되며
- Codex plugin / hook 연결이 홈 디렉터리에 준비됩니다

하지만 **이 단계만으로 모든 프로젝트에서 Ralph가 자동 개입하지는 않습니다.**
...
## Codex 안에서 어떤 식으로 반응하나

이 플러그인은 무조건 모든 프롬프트에 개입하지 않습니다.

대신 현재 프로젝트 상태와 프롬프트 의도를 보고:
...
같은 걸 판단해서 `orc plan`, `orc run`, `orc verify`, `orc resume`,
...
1. `orc init`
2. `.ralph/prd.md` 준비
   - `PRD.md`, `prd.md`, `docs/PRD.md`, `docs/prd.md`가 있으면 활용
   - 없으면 첫 프롬프트 기반으로 생성
3. `orc plan`
4. 그 다음 일반 루프로 진행
...
### Run
다음 실행 가능한 태스크를 고르고 Codex에 넘깁니다.

너무 큰 태스크는 억지로 밀어붙이지 않고,
컨텍스트 예산을 넘으면 blocked 처리 후 다시 plan 쪽으로 밀어냅니다.
...
반복 실패나 broad task는 split proposal이 생길 수 있습니다.
...
- Codex 안에서 자연스럽게 쓰기
- PRD → task graph → run → verify → resume 흐름 유지
- 검증과 상태 저장을 명시적으로 다루기
- 프로젝트별 opt-in으로 안전하게 쓰기
